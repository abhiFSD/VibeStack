...
import { getLearningSession } from "../graphql/queries";
import { updateLearningSession } from "../graphql/mutations";
function ArrayField({
  items = [],
  onChange,
  label,
...
export default function LearningSessionUpdateForm(props) {
  const {
    id: idProp,
    learningSession: learningSessionModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = learningSessionRecord
      ? { ...initialValues, ...learningSessionRecord }
      : initialValues;
    ...
    setSectionsViewed(cleanValues.sectionsViewed ?? []);
  ...
  const [learningSessionRecord, setLearningSessionRecord] = React.useState(
    learningSessionModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getLearningSession.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getLearningSession
        : learningSessionModelProp;
      setLearningSessionRecord(record);
    };
    queryData();
  }, [idProp, learningSessionModelProp]);
  React.useEffect(resetStateValues, [learningSessionRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          sectionsViewed: sectionsViewed ?? null,
          createdAt,
          updatedAt,
        ...
        try {
          ...
          await API.graphql({
            query: updateLearningSession.replaceAll("__typename", ""),
            variables: {
              input: {
                id: learningSessionRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "LearningSessionUpdateForm")}
      {...rest}
    >
      <TextField
        label="User sub"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || learningSessionModelProp)}
