...
import { getVsm } from "../graphql/queries";
import { updateVsm } from "../graphql/mutations";
export default function VsmUpdateForm(props) {
  const {
    id: idProp,
    vsm: vsmModelProp,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = vsmRecord
      ? { ...initialValues, ...vsmRecord }
      : initialValues;
    setProcess(
      typeof cleanValues.process === "string" || cleanValues.process === null
        ? cleanValues.process
        : JSON.stringify(cleanValues.process)
    );
    setInformationFlow(cleanValues.informationFlow);
    setKaizenProject(cleanValues.kaizenProject);
    setDemandData(
      typeof cleanValues.demandData === "string" ||
        cleanValues.demandData === null
        ? cleanValues.demandData
        : JSON.stringify(cleanValues.demandData)
    );
    setSummaryData(
      typeof cleanValues.summaryData === "string" ||
        cleanValues.summaryData === null
        ? cleanValues.summaryData
        : JSON.stringify(cleanValues.summaryData)
    );
    setReportID(cleanValues.reportID);
    setInventory(
      typeof cleanValues.inventory === "string" ||
        cleanValues.inventory === null
        ? cleanValues.inventory
        : JSON.stringify(cleanValues.inventory)
    );
  ...
  const [vsmRecord, setVsmRecord] = React.useState(vsmModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getVsm.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getVsm
        : vsmModelProp;
  ...
  React.useEffect(resetStateValues, [vsmRecord]);
  const validations = {
    process: [{ type: "JSON" }],
    informationFlow: [],
    kaizenProject: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          kaizenProject: kaizenProject ?? null,
        ...
        try {
          ...
          await API.graphql({
            query: updateVsm.replaceAll("__typename", ""),
            variables: {
              input: {
                id: vsmRecord.id,
                ...modelFields,
      ...
      {...getOverrideProps(overrides, "VsmUpdateForm")}
      {...rest}
    >
      <TextAreaField
        label="Process"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || vsmModelProp)}
